...
async def update_progress(
    agent_id: str, memory_node: str, step: str, progress: int, findings: str = ""
):
...
"""
    ...
    await update_progress(
        agent_id, memory_node, f"Analyzing {len(intercepted_requests)} requests", 50
    )

    # Categorize requests
    api_requests = [
        r for r in intercepted_requests if "/api/" in r["url"] or "/graphql" in r["url"]
    ]
    static_requests = [
        r
        for r in intercepted_requests
        if any(ext in r["url"] for ext in [".js", ".css", ".png", ".jpg", ".svg", ".woff"])
    ]
    other_requests = [
        r for r in intercepted_requests if r not in api_requests and r not in static_requests
    ]
...
"""
    await update_progress(
        agent_id, memory_node, "Security analysis complete", 80, security_findings
    )
    ...
    if browser.network_logs:
        ...
        await update_progress(
            agent_id, memory_node, "Response status analysis", 90, status_findings
        )
    ...
    if errors:
        ...
        await update_progress(
            agent_id, memory_node, "No console errors found", 95, "- **Console errors**: None"
        )
