...
"""
        ...
        try:
            ...
            logger.info("llm_action_decided", action=action.get("action"), reasoning=action.get("reasoning", "")[:100])
            return action
            
        except json.JSONDecodeError as e:
            logger.error("failed_to_parse_llm_action", error=str(e), raw=response.content[:500])
            return {"action": "screenshot", "reasoning": "Failed to parse LLM response, taking screenshot for context", "confidence": 50}
        except Exception as e:
            logger.error("llm_planning_error", error=str(e))
            return {"action": "complete", "reasoning": f"Error during planning: {e}", "confidence": 0}
    ...
    async def _log(self, action: Dict, result: Dict, observation: Dict) -> None:
        ...
        try:
            self.vault.update_frontmatter(self.memory_node, {
                "status": "active",
    ...
    async def _complete(self) -> None:
        ...
        # Determine result
        result = "pass" if len(self.findings) == 0 else "warning" if len(self.findings) < 3 else "fail"
