...
async def test_homepage(
    browser: BrowserAutomation, url: str, agent_id: str, memory_node: str
) -> ReportBuilder:
            ...
            [
                {
                    ...
                    "description": f"Could not load page: {result.get('error', 'Unknown')}",
                    "severity": "critical",
                }
            ],
        ...
        [
            {
                ...
                "description": f"Page loaded with status {result['status']}",
                "severity": "info",
            }
        ],
        page_metrics,
    ...
    await update_progress(
        agent_id, memory_node, "Captured homepage screenshot", 25, screenshots=[screenshot_path]
    )
        ...
        nav_findings.append(
            {
                ...
                "description": "No <nav>, <header>, or navigation classes detected",
                "severity": "high",
            }
        )
        ...
        nav_findings.append(
            {
                ...
                "description": f"Found {nav_result['count']} navigation element(s)",
                "severity": "info",
            }
        )
    ...
    report.add_section(
        "Navigation Audit",
        "pass" if nav_result["count"] > 0 else "warning",
        nav_findings,
        nav_metrics,
    )
    ...
    if main_result["count"] == 0:
        content_findings.append(
            {
                "title": "No Main Content Area",
                "description": "No <main> or .content container found",
                "severity": "medium",
            }
        )
    else:
        content_findings.append(
            {
                "title": "Main Content Present",
                "description": f"Found {main_result['count']} content area(s)",
                "severity": "info",
            }
        )
        ...
        content_findings.append(
            {
                ...
                "description": "Page lacks heading structure (h1-h6)",
                "severity": "high",
            }
        )
        report.add_recommendation(
            "Add proper heading hierarchy (h1 for main title, h2-h6 for sections)"
        )
    else:
        content_findings.append(
            {
                "title": "Heading Structure",
                "description": f"Found {header_result['count']} heading(s)",
                "severity": "info",
            }
        )
    ...
    report.add_section(
        ...
        "pass",
        content_findings,
        {"Content areas": main_result["count"], "Headings": header_result["count"]},
    )
    ...
    report.add_section(
        "Footer Check",
        "pass" if footer_result["count"] > 0 else "warning",
        [
            {
        ...
        error_findings.append(
            {
                ...
                "description": f"Found {len(errors)} error(s) in browser console",
                "severity": "high",
            }
        )
        for i, err in enumerate(errors[:5]):
            error_findings.append(
                {"title": f"Error {i+1}", "description": str(err)[:100], "severity": "high"}
            )
        ...
        error_findings.append(
            {
                ...
                "description": "No errors detected in browser console",
                "severity": "info",
            }
        )
    ...
    report.add_section(
        ...
        [
            {
                "title": "Links Found",
                "description": f"Found {links_result['count']} link(s) on page",
                "severity": "info",
            }
        ],
    ...
    report.add_section(
        ...
        [
            {
                "title": "Page Scrollable",
                "description": "Page scrolled to bottom successfully",
                "severity": "info",
            }
        ],
    )
...
async def test_navigation(
    ...
    link_findings = [
        {
            "title": "Total Links",
            "description": f"Found {links_result['count']} total links",
            "severity": "info",
        },
        {
            "title": "Internal Links",
            "description": f"Found {len(internal_links)} internal link(s)",
            "severity": "info",
        },
        {
            "title": "External Links",
            "description": f"Found {len(external_links)} external link(s)",
            "severity": "info",
        },
    ]
    ...
    for link in internal_links[:10]:  # Test up to 10
        ...
        await update_progress(
            agent_id, memory_node, f"Testing link: {link['text'][:30]}...", progress
        )
            ...
            link_results.append(
                {
                    ...
                    "url": href,
                    "status": "OK",
                    "http_status": nav_result.get("status", "OK"),
                }
            )
        else:
            link_results.append(
                {
                    ...
                    "url": href,
                    "status": "BROKEN",
                    "error": nav_result.get("error", "Error"),
                }
            )
            broken += 1
    ...
    link_test_findings = []
    if tested == 0:
        link_test_findings.append(
            {
                "title": "No Links Tested",
                "description": "No valid internal links found to test",
                "severity": "warning",
            }
        )
    else:
        link_test_findings.append(
            {
                "title": "Links Tested",
                "description": f"Tested {tested} link(s), {broken} broken",
                "severity": "info" if broken == 0 else "high",
            }
        )
        if broken > 0:
            for lr in link_results:
                if lr["status"] == "BROKEN":
                    link_test_findings.append(
                        {
                            "title": f"Broken: {lr['text'][:40]}",
                            "description": lr["url"],
                            "severity": "high",
                        }
                    )
            report.add_recommendation(
                "Fix broken internal links for better SEO and user experience"
            )
...
async def test_contact_form(
        ...
        if elements["count"] > 0:
            ...
            if selector.startswith("a"):
                ...
                report.add_section(
                    ...
                    [
                        {
                            "title": "Contact Link Found",
                            "description": f"Clicked contact link: `{selector}`",
                            "severity": "info",
                        }
                    ],
                )
                ...
                report.add_section(
                    ...
                    [
                        {
                            "title": "Form Found",
                            "description": f"Found form via selector: `{selector}`",
                            "severity": "info",
                        }
                    ],
                )
            ...
            [
                {
                    ...
                    "description": "Checked links, forms, and contact sections - none found",
                    "severity": "medium",
                }
            ],
        )
    ...
    if form_result["success"]:
        ...
        if len(fields) == 0:
            ...
            form_findings.append(
                {
                    ...
                    "description": f"Found {len(fields)} field(s)",
                    "severity": "info",
                }
            )
                ...
                form_findings.append(
                    {
                        ...
                        "description": f"{len(required)} field(s) marked as required",
                        "severity": "info",
                    }
                )
            else:
                form_findings.append(
                    {
                        ...
                        "description": "Form accepts empty submissions",
                        "severity": "medium",
                    }
                )
            ...
            has_name = any(
                "name" in f["name"].lower() or "name" in f.get("placeholder", "").lower()
                for f in fields
            )
            has_email = any(
                "email" in f["name"].lower() or "email" in f.get("placeholder", "").lower()
                for f in fields
            )
            has_message = any(
                "message" in f["name"].lower() or "message" in f.get("placeholder", "").lower()
                for f in fields
            )
                ...
                form_findings.append(
                    {
                        ...
                        "description": "No name field detected",
                        "severity": "low",
                    }
                )
            if not has_email:
                ...
                form_findings.append(
                    {
                        ...
                        "description": "No message/comment field detected",
                        "severity": "medium",
                    }
                )
    else:
    ...
    report.add_section(
        "Form Analysis",
        "pass" if form_result["success"] and len(form_result.get("fields", [])) > 0 else "warning",
        form_findings,
        form_metrics,
    )
    ...
    await update_progress(
        agent_id, memory_node, "Contact form screenshot captured", 90, screenshots=[screenshot_path]
    )
...
async def test_accessibility(
    ...
    report.add_section(
        "Page Load",
        "pass",
        [{"title": "Page Loaded", "description": url, "severity": "info"}],
        {"URL": url, "Status": result["status"]},
    )
    await update_progress(agent_id, memory_node, "Page loaded", 15)
            ...
            img_findings.append(
                {
                    ...
                    "description": f"{len(no_alt)} of {images['count']} image(s) lack alt text",
                    "severity": "high",
                }
            )
            report.add_recommendation(
                f"Add alt text to {len(no_alt)} image(s) for screen reader compatibility"
            )
        else:
            img_findings.append(
                {
                    "title": "All Images Have Alt Text",
                    "description": f"All {images['count']} image(s) have alt attributes",
                    "severity": "info",
                }
            )
    else:
        img_findings.append(
            {"title": "No Images", "description": "No images found on page", "severity": "info"}
        )

    report.add_section(
        "Image Accessibility", "pass", img_findings, {"Total images": images["count"]}
    )
    ...
    if headings["count"] == 0:
        ...
        if h1_count == 0:
            heading_findings.append(
                {
                    "title": "Missing H1",
                    "description": "No h1 heading found (required for screen readers)",
                    "severity": "high",
                }
            )
            ...
            heading_findings.append(
                {
                    "title": "Multiple H1s",
                    "description": f"Found {h1_count} h1 headings (should be exactly 1)",
                    "severity": "medium",
                }
            )
        else:
    ...
    if aria_elements["count"] > 0:
        aria_findings.append(
            {
                "title": "ARIA Attributes",
                "description": f"Found {aria_elements['count']} element(s) with ARIA attributes",
                "severity": "info",
            }
        )
    else:
    ...
    report.add_section(
        "ARIA Check",
        "pass" if aria_elements["count"] > 0 else "warning",
        aria_findings,
    ...
    if forms["success"] and forms["count"] > 0:
        ...
        if labeled < len(fields):
            form_findings.append(
                {
                    ...
                    "description": f"{len(fields) - labeled} of {len(fields)} field(s) lack labels",
                    "severity": "high",
                }
            )
            ...
            form_findings.append(
                {
                    "title": "All Fields Labeled",
                    "description": f"All {len(fields)} field(s) have labels",
                    "severity": "info",
                }
            )
    ...
    if form_findings:
        report.add_section(
            "Form Labels", "pass", form_findings, {"Total fields": forms.get("count", 0)}
        )
        ...
        [
            {
                ...
                "description": "Tab navigation supported (browser default)",
                "severity": "info",
            }
        ],
...
async def test_responsive(
    ...
    for i, (width, height, name) in enumerate(viewports):
        ...
        if result["success"]:
            # Take screenshot
            screenshot_path = (
                f"obsidian_vault/Screenshots/{agent_id}_{name.lower().replace(' ', '_')}.png"
            )
            await browser.screenshot(screenshot_path)
            report.add_screenshot(screenshot_path)
    ...
    report.add_section(
        ...
        "pass",
        viewport_findings,
        {"Tested viewports": len(viewports), "Screenshots": len(report.report.screenshots)},
    )
...
async def run_agent(agent_id: str, memory_node: str):
    ...
    try:
        ...
        if "test navigation" in objective_lower or "test nav" in objective_lower:
            ...
            "full suite" in objective_lower
            or "comprehensive" in objective_lower
            or "all tests" in objective_lower
        ):
