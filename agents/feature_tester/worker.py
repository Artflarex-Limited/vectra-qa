...
class FeatureTesterWorker:
    ...
    def _parse_credentials(self) -> Dict[str, str]:
        ...
        username_match = re.search(r'username[:\s]+["\']?([^"\'\s]+)["\']?', self.objective, re.IGNORECASE)
        password_match = re.search(r'password[:\s]+["\']?([^"\'\s]+)["\']?', self.objective, re.IGNORECASE)
        if username_match:
    ...
    async def run(self) -> None:
        ...
        if not url:
            ...
            await self._save_results({
                "status": "error",
                "error": "No URL found in objective"
            })
            return
        ...
        for test_name, test_result in results.items():
            if isinstance(test_result, dict) and "findings" in test_result:
                for finding in test_result["findings"]:
                    findings_summary.append(f"- [{finding.get('severity', 'info').upper()}] {finding.get('title', 'Unknown')}: {finding.get('description', '')}")
