...
"""
            ...
            logger.info("state_saved", agent_count=len(state["agents"]), path=str(STATE_BACKUP_PATH))
    ...
    def restore_state(self) -> List[Dict[str, Any]]:
        ...
        try:
                ...
                if memory_node:
                    try:
                        self.vault.update_frontmatter(memory_node, {
                            "status": "orphaned",
                            "orphaned_at": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S") + "Z",
            ...
            self.vault.update_frontmatter("Global/Agent_State_Backup.md", {
                "status": "restored",
        ...
        for agent in orphaned:
            if agent_ids is None or agent["agent_id"] in agent_ids:
                try:
                    ...
                    self.vault.update_frontmatter(agent["memory_node"], {
                        "status": "terminated",
                        "terminated_at": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S") + "Z",
