        ...
        async with self._semaphore:
            async with self._lock:
                ...
                while self._pool:
                    ...
                    if browser.page and not browser.browser.is_connected() if hasattr(browser.browser, 'is_connected') else True:
...
class AgentResourceTracker:
    ...
    def record_step(self, agent_id: str) -> bool:
        ...
        if self._agents[agent_id]["steps"] > MAX_AGENT_STEPS:
            logger.warning("agent_step_limit_exceeded", agent_id=agent_id, steps=self._agents[agent_id]["steps"])
            return False
    ...
    def record_llm_call(self, agent_id: str) -> bool:
        ...
        if self._llm_calls[agent_id] > MAX_LLM_CALLS_PER_AGENT:
            logger.warning("agent_llm_limit_exceeded", agent_id=agent_id, calls=self._llm_calls[agent_id])
            return False
    ...
    def check_timeout(self, agent_id: str) -> bool:
        ...
        elapsed = (datetime.now(timezone.utc) - self._agents[agent_id]["started_at"]).total_seconds()
        max_duration = self._agents[agent_id]["max_duration"]
        
        if elapsed > max_duration:
            logger.warning("agent_timeout", agent_id=agent_id, elapsed=elapsed, max_duration=max_duration)
            return True
