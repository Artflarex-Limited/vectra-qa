...
class PerformanceTester:
    ...
    async def test_performance(
        ...
        try:
            ...
            if browser.page:
                ...
                # Calculate FCP
                fcp_entry = next((p for p in paint if p.get("name") == "first-contentful-paint"), None)
                if fcp_entry:
    ...
    async def _run_lighthouse(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Run lighthouse
            output_file = f"/tmp/lighthouse_{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}.json"
