...
class AuthFlowTester:
    ...
    async def test_login_flow(
        ...
        try:
            ...
            if not result["success"]:
                self._add_finding("Navigation Failed", f"Cannot load login page: {result.get('error')}", "critical")
                return self._build_result("fail", start_time)
            
            # Check for HTTPS
            if not login_url.startswith("https://"):
                self._add_finding("Insecure Login Page", "Login page not served over HTTPS", "critical")
            ...
            if password_type != "password":
                self._add_finding("Password Field Not Masked", f"Password input type is '{password_type}' instead of 'password'", "critical")
            
            # Check for autocomplete
            autocomplete = await password_field.get_attribute("autocomplete")
            if autocomplete not in ["current-password", "new-password"]:
                self._add_finding("Password Autocomplete", "Password field missing proper autocomplete attribute", "medium")
            ...
            if session_cookie:
                ...
                if not session_cookie.get("httpOnly"):
                    self._add_finding("Session Cookie Not HttpOnly", "Session cookie accessible to JavaScript", "high")
                
                if not session_cookie.get("secure"):
                    self._add_finding("Session Cookie Not Secure", "Session cookie sent over HTTP", "high")
                
                if session_cookie.get("sameSite") not in ["Strict", "Lax"]:
                    self._add_finding("Session Cookie SameSite", f"SameSite is '{session_cookie.get('sameSite')}'", "medium")
            else:
                self._add_finding("No Session Cookie", "No session cookie found after login", "warning")
            
            # Check for success
            current_url = self.browser.page.url
            logged_in = success_indicator is None or await self._check_element_exists(success_indicator)
            ...
            if logged_in and current_url != login_url:
                ...
                local_storage = await self.browser.page.evaluate("() => JSON.stringify(localStorage)")
                if "token" in local_storage.lower() or "jwt" in local_storage.lower():
                    self._add_finding("Token in localStorage", "Authentication token stored in localStorage (consider httpOnly cookies)", "medium")
            else:
                self._add_finding("Login May Have Failed", f"Still on or returned to {current_url}", "warning")
            
            return self._build_result("pass" if not any(f["severity"] == "critical" for f in self.findings) else "warning", start_time)
    ...
    async def test_logout_flow(
        ...
        try:
            ...
            if session_cookie:
                self._add_finding("Session Still Active", "Session cookie present after logout", "high")
            else:
    ...
    async def _find_element(self, selector: str, name: str) -> Optional[Any]:
        ...
        try:
            ...
            if element:
                ...
                self._add_finding(f"{name} Not Found", f"Selector '{selector}' not found", "critical")
                return None
        except Exception as e:
