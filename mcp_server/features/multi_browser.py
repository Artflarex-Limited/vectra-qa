    ...
    async def test_all_browsers(
        ...
        for browser_name in test_browsers:
            ...
            try:
                ...
                playwright = await browser.playwright.start() if hasattr(browser, 'playwright') else None
                ...
                self.results[browser_name] = {
                    ...
                    "duration_seconds": (datetime.now(timezone.utc) - browser_start).total_seconds()
                }
        ...
        for browser_name, browser_result in result.get("results", {}).items():
            if browser_result.get("status") == "pass":
                ...
                screenshot_path = next(
                    (f.get("screenshot_path") for f in findings if f.get("screenshot_path")),
                    None
                )
                if screenshot_path:
