...
class AccessibilityTester:
    ...
    async def test_accessibility(
        self, browser, url: str, rules: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def _manual_accessibility_check(self, browser, start_time: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            status = (
                ...
                else "warning" if any(f["severity"] == "high" for f in self.findings) else "pass"
            )
            ...
            self.findings.append(
                {"title": "Manual Check Error", "description": str(e), "severity": "critical"}
            )
            return self._build_result("fail", start_time)

    def _map_axe_impact(self, impact: str) -> str:
        """Map axe-core impact to our severity levels."""
        mapping = {"critical": "critical", "serious": "high", "moderate": "medium", "minor": "low"}
