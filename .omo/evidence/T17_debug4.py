import subprocess
import sys
import time
import os

env = os.environ.copy()
env["OBSIDIAN_VAULT_PATH"] = "/tmp/vectra_test_vault"
env["COMMAND_CENTER_PORT"] = "3000"

server = subprocess.Popen(
    [sys.executable, "-m", "uvicorn", "command_center.main:app", "--host", "0.0.0.0", "--port", "3000"],
    cwd="/home/bugra/Documents/projects/vectra-qa",
    env=env,
    stdout=subprocess.PIPE,
    stderr=subprocess.PIPE,
)

time.sleep(3)

try:
    from playwright.sync_api import sync_playwright

    with sync_playwright() as p:
        browser = p.chromium.launch(headless=True)
        page = browser.new_page(viewport={"width": 1280, "height": 800})
        
        page.on("console", lambda msg: print(f"CONSOLE [{msg.type}]: {msg.text}"))
        page.on("pageerror", lambda err: print(f"PAGE ERROR: {err}"))
        
        page.goto("http://localhost:3000")
        time.sleep(3)
        
        page.screenshot(path="/home/bugra/Documents/projects/vectra-qa/.omo/evidence/T17_debug5.png")
        
        # Try to call toggle via JS
        result = page.evaluate("""
            try {
                toggleChatPanel();
                return 'ok';
            } catch (e) {
                return e.message;
            }
        """)
        print(f"toggleChatPanel result: {result}")
        
        time.sleep(1)
        panel = page.locator("#chat-panel-container")
        print(f"Panel classes: {panel.get_attribute('class')}")
        
        browser.close()

finally:
    server.terminate()
    try:
        server.wait(timeout=5)
    except:
        server.kill()
