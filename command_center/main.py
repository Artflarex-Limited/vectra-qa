...
async def get_chat_history(limit: int = 50):
    ...
    try:
        ...
        return JSONResponse(
            status_code=500,
            content={"error": f"Failed to load chat history: {str(e)}"}
        )
...
async def execute_chat_plan(url: str = Form(...), tests: str = Form(...)):
    ...
    try:
        ...
        if not valid_tests:
            return JSONResponse(
                status_code=400,
                content={"error": "No valid test types specified"}
            )
...
async def interpret_test_results(agent_id: str):
    ...
    try:
        ...
        return JSONResponse(
            status_code=404,
            content={"error": "Test result not found"}
        )
        ...
        return JSONResponse(
            status_code=500,
            content={"error": f"Interpretation error: {str(e)}"}
        )
