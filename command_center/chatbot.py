...
Intent:"""
        ...
        try:
            response = self.llm.complete(
                model=CHATBOT_MODEL,
                messages=[
                    {"role": "system", "content": "You are an intent classifier. Respond with exactly one word: chat, plan_tests, or interpret_results."},
        ...
        if any(kw in msg_lower for kw in ["result", "found", "mean", "issue", "problem", "fix", "what did"]):
...
Respond with a comma-separated list of test IDs (e.g., "homepage,navigation" or "full"):"""
            ...
            try:
                response = self.llm.complete(
                    model=CHATBOT_MODEL,
                    messages=[
                        {"role": "system", "content": "You are a test planner. Respond only with comma-separated test IDs."},
                        {"role": "user", "content": prompt}
                    ],
                    temperature=0.2,
                    max_tokens=50
        ...
        return {
            "url": url,
            "tests": tests,
            "test_configs": [TEST_TYPES[t] for t in tests]
        }
            ...
            for section in result_data["sections"]:
                ...
                if "findings" in section:
                    for finding in section["findings"]:
                        context += f"  - {finding.get('severity', 'info')}: {finding.get('title', '')}\n"
...
Format with markdown for readability."""
        ...
        try:
            response = self.llm.complete(
                model=CHATBOT_MODEL,
                messages=[
                    {"role": "system", "content": "You are a QA analyst interpreting automated test results."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.7,
                max_tokens=2500
    ...
    async def stream_response(self, message: str, history: List[Dict] = None) -> AsyncGenerator[str, None]:
    ...
    def process_message(self, message: str) -> Dict[str, Any]:
        ...
        if intent == "plan_tests":
            ...
            agent_id_match = re.search(r'agent[_-]?([a-z]+-\d{14}-[a-f0-9]{6})', message, re.IGNORECASE)
